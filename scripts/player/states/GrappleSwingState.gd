...
func enter(payload: Dictionary = {}):
	...
	#grappling_hook.get_swinging_body().apply_central_force(player.velocity)


func physics_update(delta: float):
	print(grappling_hook.swinging_body.linear_velocity)
	if not has_set_initial_velocity:
		var initial_velocity = player.velocity
		print("MAGNITUDE / MAX_LENGTH: ", grappling_hook.get_pendulum_length(), " / ", 16.0)
		initial_velocity.y = clamp(initial_velocity.y, -player.maximum_falling_speed, -player.maximum_falling_speed / 2)
		#initial_velocity.y /= grappling_hook.get_pendulum_length()
		
		grappling_hook.get_swinging_body().apply_central_impulse(initial_velocity)
		has_set_initial_velocity = true
	else:
		player.set_global_position(grappling_hook.get_swinging_body().global_position)
		player.set_up_vector(grappling_hook.get_tension_vector())
...
func handle_input(event):
	if event.is_action_pressed("jump"):
		transition_to.emit(self, "Airborne", { "ground_jump": true, "initial_velocity": grappling_hook.swinging_body.linear_velocity / 4 })
